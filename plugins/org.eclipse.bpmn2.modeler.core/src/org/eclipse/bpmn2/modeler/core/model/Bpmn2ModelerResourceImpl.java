...
public class Bpmn2ModelerResourceImpl extends Bpmn2ResourceImpl {
	...
	// this.getDefaultLoadOptions().put(XMLResource.OPTION_LAX_FEATURE_PROCESSING,
	// true);
	// this.getDefaultLoadOptions().put(XMLResource.OPTION_LAX_WILDCARD_PROCESSING,
	// true);
	// this.getDefaultLoadOptions().put(XMLResource.OPTION_RECORD_UNKNOWN_FEATURE,
	// true);
	// this.getDefaultLoadOptions().put(XMLResource.OPTION_ANY_TYPE,
	// BpmnDiPackage.eINSTANCE.getBPMNPlane());
	this.getDefaultLoadOptions().put(XMLResource.OPTION_USE_XML_NAME_TO_FEATURE_MAP, xmlNameToFeatureMap);

	// only necessary if this resource will not be added to a ResourceSet
	// instantly
	 ...
	 * Default is to use that "targetNamespace" attribute in the
	 * <definitions> element.
	 * 
	 * @param defs
	 * @return
	...
	public void endDocument() {
	    ...
	    // We'll need that prefix to qualify data types defined in the type
	    // language.
	    ...
	    // Load all of the Imports and generate Interfaces, Operations,
	    // Messages, Faults and ItemDefinitions
		...
		// Set the diagram's description AFTER it has been completely
		// populated
	 ...
	 *            the object referencing an external object.
	 * @param referencedObject
	 *            the referenced object contained in another file in the
	 *            project.
	 * @param id
	...
	private boolean shouldResolveExternals(EObject referencingObject, EObject referencedObject, String id) {
		...
		boolean doit = MessageDialog.openQuestion(new Shell(), Messages.Bpmn2ModelerResourceSetImpl_External_Reference_Found_Title,
			NLS.bind(Messages.Bpmn2ModelerResourceSetImpl_External_Reference_Found_Message,
				new Object[] { referencedObject.eClass().getName(), id, ModelUtil.getLabel(referencingObject) + " \"" + ModelUtil.getName((BaseElement) referencingObject) + "\"", //$NON-NLS-1$ //$NON-NLS-2$
					path.toString() }));
		return doit;
	    } catch (Exception e) {
	    }
	...
	protected EStructuralFeature getFeature(EObject object, String prefix, String name, boolean isElement) {
		...
		if (feature != null) {
		    ...
		    // "+object.eClass().getName()+"."+name+" in pkg
		    // "+pkg.getName());
		    return feature;
		}
	    }
	...
	protected EObject createObjectFromFeatureType(EObject peekObject, EStructuralFeature feature) {
		...
		// ModelDecoratorAdapter mda = AdapterUtil.adapt(pkg,
		// ModelDecoratorAdapter.class);
	...
	protected EObject validateCreateObjectFromFactory(EFactory factory, String typeName, EObject newObject, EStructuralFeature feature) {
		...
		if (id != null) {
		    // if this is a CustomElement we need to discard this object
		    // and construct it
		    // properly such that all extension attributes and elements
		    // are created and
		    // initialized.
		    newObject = Bpmn2ModelerFactory.create(xmlResource, newObject.eClass(), new KeyValue(GraphitiConstants.CUSTOM_ELEMENT_ID, id));
		    handleObjectAttribs(newObject);
		}
	    }
	...
	protected void setFeatureValue(EObject object, EStructuralFeature feature, Object value, int position) {
		...
		// ExtendedPropertiesAdapter.adapt(object);
	...
	protected void handleObjectAttribs(EObject obj) {
		...
		for (int i = 0, size = attribs.getLength(); i < size; ++i) {
			...
			// create an ns prefix in the prefix map for this
			// default namespace
			// and qualify any qnameFeatures contained in this
			// object...
	    ...
	    /*
	     * it's already too late at this point to identify targetNamespace -
	     * see createDocumentRoot() else if (obj instanceof Definitions) {
	     * // Fetch the targetNamespace from Definitions object: // this is
	     * where we allow the TargetRuntime extension to provide // its own
	     * targetNamespace if the file doesn't define one. // The
	     * targetNamespace determines the TargetRuntime and therefore // how
	     * the file is handled, since the TargetRuntime may have special //
	     * processing that needs to be done during loading. Therefore, //
	     * it's important that we have a targetNamespace after the
	     * <definitions> // element is parsed, or the remainder of the file
	     * may not be processed // correctly. targetNamespace =
	     * getTargetNamespace((Definitions)obj); // if targetNamespace is
	     * not provided, use "tns" from file if (targetNamespace==null &&
	     * tns!=null && !tns.isEmpty()) targetNamespace = tns; // if that
	     * didn't work, try "xmlns" if (targetNamespace==null && xmlns!=null
	     * && !xmlns.isEmpty()) targetNamespace = xmlns; // if that still
	     * didn't work use the default namespace if (targetNamespace==null)
	     * targetNamespace =
	     * TargetRuntime.getDefaultRuntime().getRuntimeExtension().
	     * getTargetNamespace(null);
	     * ((Bpmn2ModelerXmlHelper)helper).initializeTargetRuntime(
	     * targetNamespace);
	     * 
	     * }
	     */
	...
	protected EObject createDocumentRoot(String prefix, String uri, String name, EFactory eFactory, boolean top) {
	    // Fetch the targetNamespace from DocumentRoot (<definitions>)
	    // object:
	    // this is where we allow the TargetRuntime extension to provide
	    // its own targetNamespace if the file doesn't define one, by
	    // overriding
	    // the Bpmn2ModelerXmlHandler#getTargetNamespace(Definitions)
	    // method.
	    ...
	    // it's important that we have a targetNamespace after the
	    // <definitions>
	    // element is parsed, or the remainder of the file may not be
	    // processed
	    // correctly.
	    //
	    // TargetNamespace identification needs to happen very early in the
	    // resource
	    // load process, sometime before the first EObject is created by
	    // Bpmn2ModelerFactory
	...
	protected void setValueFromId(EObject object, EReference eReference, String ids) {
		...
		// This reference might be a QName (according to the BPMN2
		// spec!)
		// or it might not, in the case of some jBPM Java type
		// references.
		int i = ids.indexOf(":"); //$NON-NLS-1$
		if (i > 0) {
		    // if the ID string is a QName, try to resolve and load the
		    // object
			...
			// so remove the prefix to avoid confusing the
			// XMLHandler
			ids = localname;
		    } else {
			// this thing is in another namespace, possibly in an
			// external document
		...
		if (value != null && eReference.getEType().isInstance(value)) {
			...
			String msg = NLS.bind(Messages.Bpmn2ModelerResourceImpl_Invalid_Reference, new Object[] { object, eReference, value });
			IStatus s = new Status(Status.ERROR, Activator.PLUGIN_ID, msg, e);
	    ...
	    // "Official" BPMN 2.0 xsd says these SHOULD be QNames, the bpmn2
	    // EMF
	    // model has these defined as "resolveProxies=false" which means
	    // they
	...
	public void startElement(String uri, String localName, String name) {
	    ...
	    // // create all model extensions for this target runtime so that
	    // all
	    // // extension attributes and elements are available during
	    // loading.
	    // TargetRuntime rt = TargetRuntime.getCurrentRuntime();
	    // rt.createModelExtensions(peekObject.eResource());
	    // }
	    if (peekObject != null && peekObject.getClass() == Expression.class) {
		// If the element is an Expression (not a FormalExpression) then
		// use the CDATA
		// as the body of a Formal Expression (because Expression does
		// not have a body)
	...
	public void endElement(String uri, String localName, String name) {
	    ...
	    if (peekObject != null && peekObject.getClass() == Expression.class) {
		// if the element is an Expression, replace it with a
		// FormalExpression and set
	...
	protected class Bpmn2ModelerXMLString extends XMLString {
		...
		// and schemaLocation attributes for the XSI namespace if the
		// prefix
		// is anything other than "xsi". The EMF serializers rely on the
		// fact
		// that the XSI namespace prefix is ALWAYS "xsi" and they WILL
		// create
		// a duplicate namespace declaration if one already existed
		// under a
		// different prefix. This would result a nasty warning from the
		// parser.
	...
	protected boolean shouldSaveFeature(EObject o, EStructuralFeature f) {
	    ...
	    // we also want to store x and y with value zero, would be skipped
	    // because of default value otherwise
	    ...
	    // don't serialize the "body" attribute of FormalExpressions because
	    // the expression text
	    // is already in the CDATA section of the <bpmn2:expression>
	    ...
	    // both a "from" and "to" expression and they may not be empty
	    // strings.
	...
	protected void saveContainedMany(EObject o, EStructuralFeature f) {
		...
		// Sort the Definitions Root Elements to avoid forward
		// references
	...
	public class Bpmn2Lookup extends XMLSaveImpl.Lookup {
	     ...
	     * Subclasses should override this behavior. The default
	     * implementation simply returns the original list.
	     * 
	     * @param cls
	     *            - EClass whose features need to be reordered
	     * @param featureList
	     *            - the original feature list as provided by the
	     *            Bpmn2Package
	    ...
	    /**
	     * Change the serialization order of features for a given EClass.
	     * The string array "featureNames" specifies a new ordering for some
	     * or all of the features in the feature list; these names must be
	     * contiguous in the original list. For example, given the following
	     ...
	     * "b" "a" "e" "d"
	     * 
	     * because the two sets "b", "a" and "e", "d" are not contiguous.
	     * The correct way of specifying this is:
	     * 
	     * "b" "a" "c" "e" "d"
	     * 
	     * Alternatively, the client could call this method twice, the first
	     * time with the first set ("b" and "a") and a second time with the
	     * second set ("e" and "d).
	     * 
	     * @param cls
	     * @param featureList
    ...
    /**
     * QName handler to make create URIs out of the fragment, which is the local
     * part of the QName
     * 
	...
	public URI deresolve(URI uri) {
		...
		// return just fragment (i.e. without the '#') but only if local
		// reference
	    ...
	    if ((uri.isPlatformResource() && baseURI.isPlatformResource()) || (uri.isFile() && baseURI.isFile())) {
	...
	public String convertQNameToUri(String qName) {
		...
		Resource r = ((Bpmn2ModelerResourceSetImpl) rs).getResource(uri, true, "wsdl"); // the //$NON-NLS-1$
												// only
												// problem
												// here...
	...
	// List of all EReferences that are defined as type="xsd:QName" in the
	// BPMN 2.0 Schema
	// This information is not represented in the MDT BPMN2 project
	// metamodel.
	...
	public String getIDREF(EObject obj) {
		...
		if (isQNameFeature) {
		    // Don't use targetNamespace prefixes for BPMN2 object ID
		    // references
	...
	/**
	 * Returns the targetNamespace defined in the <definitions> root
	 * element.
	 * 
	 ...
	 * 
	 * @return null if the document does not define a targetNamespace, an
	 *         empty string if there is a targetNamespace, but no prefix has
	 *         been defined for it, or the prefix for the targetNamespace
	 * 
	 *         Examples:
	 * 
	 *         <bpmn2:definitions tns="http://eclipse.org/example"
	 *         targetNamespace="http://eclipse.org/example"/> return "tns"
	 * 
	 *         <bpmn2:definitions targetNamespace=
	 *         "http://eclipse.org/example"/> returns ""
	 * 
	 *         <bpmn2:definitions tns="http://eclipse.org/example"/> returns
	 *         null
	 */
	 ...
	 * 
	 * @param s
	 *            - a non-colonized name string
	 * @return the string "s" prefixed with the NS prefix for the
	 *         targetNamespace.
	 */
	...
	public void setValue(EObject object, EStructuralFeature feature, Object value, int position) {
	    // fix some kind of bug which causes duplicate entries in objects
	    // that have
	    // mutual reference lists.
	    if (object != null && feature != null && object.eClass() != null && feature == object.eClass().getEStructuralFeature(feature.getFeatureID())) {
	    ...
	    // // search for the attribute in the target runtime's Custom Task
	    // and
	...
	public EStructuralFeature getFeature(EClass eClass, String nsURI, String name, boolean isElement) {
	    ...
	    // I'm still not convinced that getFeature() shouldn't simply return
	    // the feature
	    // from the given EClass instead of searching the EPackage of the
	    // Resource being
	    // loaded (if the EClass has a feature with that name of course).
	    EStructuralFeature feature = null;
	    EPackage pkg = eClass.getEPackage();
	    if (pkg != Bpmn2Package.eINSTANCE && pkg != BpmnDiPackage.eINSTANCE && pkg != DcPackage.eINSTANCE && pkg != DiPackage.eINSTANCE
