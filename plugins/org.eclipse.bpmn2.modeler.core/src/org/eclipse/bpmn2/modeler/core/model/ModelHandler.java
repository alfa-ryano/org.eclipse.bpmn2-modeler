...
public class ModelHandler {
	...
	if (domain != null) {
		...
		protected void doExecute() {
		    ...
		    // nonInitiatingProcess.setName(name+" Non-initiating
		    // Process");
		    // nonInitiatingParticipant.setProcessRef(nonInitiatingProcess);
	...
	if (bpmnDiagram == null) {
	    // return the first Collaboration or Choreography in the model
	    // hierarchy
	    List<RootElement> rootElements = getDefinitions(resource).getRootElements();
	    for (RootElement element : rootElements) {
		// yeah, Collaboration and Choreography are both instanceof
		// Collaboration...
    ...
    // TODO: Move all of this model handler crap into BPMN2PersistencyBehavior
    // where it belongs
    void loadResource() {
	try {
	    resource.load(null);
		...
		for (Resource.Diagnostic error : resource.getErrors()) {
			...
			String message = Messages.ModelHandler_20 + stringValue + Messages.ModelHandler_21 + "\"" + iv.getFeature().getName() + "\""; //$NON-NLS-1$ //$NON-NLS-2$
    ...
    /**
     * getFlowElementContainer - Return the flow-element container object of the
     * specified object.
     * 
     * @param o
     * @return
	...
	if (o instanceof BPMNDiagram) {
		...
		// Find an eligible Process for this FlowElement, one that is
		// not referenced by a Pool
