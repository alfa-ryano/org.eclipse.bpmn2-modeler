...
import org.eclipse.epsilon.cbp.event.BPMN2ChangeEventAdapter;
...
public class DefaultBPMN2Editor extends DiagramEditor implements IPreferenceChangeListener {
    ...
    // ALFA
    protected BPMN2ChangeEventAdapter changeEventAdapter;
    // END ALFA
     ...
     */
    protected Bpmn2DiagramEditorInput createNewDiagramEditorInput(IEditorInput input, Bpmn2DiagramType diagramType, String targetNamespace) throws PartInitException {
    ...
    /**
     * Bypasses Graphiti's Persistency Behavior code and save only the BPMN2
     * model resource. This is only used after a successful Import if the BPMN2
     * model was changed in any way, e.g. missing DI elements were added.
     */
	...
	try {
		...
		// If the bpmn file was missing DI elements, they would have
		// been added by the importer
	...
	if (targetRuntime == null && input != null) {
	    // If the project has not been configured for a specific runtime
	    // through the "BPMN2"
	    // project properties page (i.e. the target is "None") then allow
	    // the runtime extension
	    // plug-ins an opportunity to identify the given process file
	    // contents as their own.
	    // If none of the plug-ins respond with "yes, this file is targeted
	    // for my runtime",
	    // then use the "None" as the extension. This will configure the
	    // BPMN2 Modeler with
	...
	if (pr != null) {
	    ...
	    Bpmn2ToolBehaviorProvider toolBehaviorProvider = (Bpmn2ToolBehaviorProvider) getDiagramTypeProvider().getCurrentToolBehaviorProvider();
	...
	if (!selectionChanging && selection instanceof IStructuredSelection) {
		...
		// Filter out label shapes as these are controlled by their
		// ContainerShape owners
		// For example a "Select All" key action would select Labels as
		// well.
		...
		super.selectionChanged(part, filteredSelection); // Graphiti's
	...
	// FIXME: this should be done in the figure's UpdateFeature or
	// LayoutFeature, not here.
	...
	// BusinessObjectUtil.getFirstBaseElement(shape);
	// if (baseElement instanceof Participant || baseElement instanceof
	// Lane)
	// continue;
	...
	// ((Activity)baseElement).getBoundaryEventRefs()) {
	// if (be==boundaryEvent) {
	...
	if (event.getKey().contains(Bpmn2Preferences.PREF_SHOW_ADVANCED_PROPERTIES) || event.getKey().contains("/" + Bpmn2Preferences.PREF_TOOL_PROFILE + "/")) { //$NON-NLS-1$ //$NON-NLS-2$
	    // get rid of cached Property Tab Descriptors
	    if (tabDescriptorProvider instanceof PropertyTabDescriptorProvider)
		((PropertyTabDescriptorProvider) tabDescriptorProvider).disposeTabDescriptors(bpmnResource);
	}
	...
	if (event.getKey().contains("/" + Bpmn2Preferences.PREF_SHAPE_STYLE + "/")) { //$NON-NLS-1$ //$NON-NLS-2$
		...
		protected void doExecute() {
		    ...
		    // Collect all PictogramElements and their corresponding
		    // GraphicsAlgorithms
			...
			// The Business Object class name must match the
			// ShapeStyle type
			if (be != null && be.eClass().getName().equals(name)) {
			    ...
			    // If the ShapeStyle for this BaseElement has
			    // already
    ...
    /**
     * helper class used to handle setInput() of the editor, because of it's
     * complexity split it up in own class, which may be overridden and
     * customized
     *
     * @author Flavio Donz
     *
	...
	protected ResourceSet initializeResourceSet(IEditorInput input, DefaultBPMN2Editor editor) {
	    // Determine which Target Runtime to use for this input and
	    ...
	    // Tell the TargetRuntime about the ResourceSet. This allows the
	    // TargetRuntime to provide its
	...
	public void postSetInput(IEditorInput input, DefaultBPMN2Editor editor) {
	    // Hook a transaction exception handler so we can get diagnostics
	    // about EMF validation errors.
	    editor.getEditingDomainListener();

	    // ALFA
	    Resource resource = getResourceSet().getResource(modelUri, false);
	    changeEventAdapter = new BPMN2ChangeEventAdapter((Bpmn2Resource) resource);
	    BPMN2ChangeEventAdapter adapter = changeEventAdapter;
	    resource.eAdapters().add(adapter);
	    // END ALFA

	    // This does the actual loading of the resource.
	    // TODO: move the loading code to BPMN2PersistencyBehavior where it
	    // belongs,
...
//	    editorInput.setInitialDiagramType(Bpmn2DiagramType.NONE);
	...
	protected void importDiagram(IEditorInput input, final DefaultBPMN2Editor editor) {
		...
		// Import the BPMNDI model that creates the Graphiti shapes,
		// connections, etc.
    ...
    public BPMN2ChangeEventAdapter getChangeEventAdapter() {
	return changeEventAdapter;
    }

    public void setChangeEventAdapter(BPMN2ChangeEventAdapter changeEventAdapter) {
	this.changeEventAdapter = changeEventAdapter;
    }
